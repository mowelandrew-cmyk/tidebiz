...
const PLAN_BADGE = { pro: '⭐ Pro', max: '✨ Max' }
        ...
        {filtered.map(u => (
          <div key={u.id} className="card flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {PLAN_BADGE[u.plan] && (
                <p className="text-xs text-gray-500 mt-0.5">{PLAN_BADGE[u.plan]}</p>
              )}
            </div>
            <button
              onClick={() => openDm(u)}
              className="px-4 py-1.5 rounded-xl border border-accent text-accent text-xs font-semibold hover:bg-accent/10 transition-colors shrink-0"
            >
              Message
            </button>
          </div>
      ...
      {dmOpen && (
        <div className="fixed inset-0 z-50 bg-gray-900 flex flex-col">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-800 shrink-0">
            <button
              onClick={closeDm}
              className="text-gray-400 hover:text-white text-xl leading-none mr-1"
            >
              ←
            </button>
            ...
            <div>
              ...
              {PLAN_BADGE[dmUser?.plan] && (
                <p className="text-xs text-gray-500">{PLAN_BADGE[dmUser.plan]}</p>
              )}
            </div>
          </div>
          ...
          <div className="shrink-0 border-t border-gray-800 px-4 py-3 flex gap-2 items-center">
            <input
              className="flex-1 bg-gray-800 rounded-2xl px-4 py-2.5 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-accent"
            ...
            <button
              ...
              className="w-10 h-10 rounded-full bg-accent flex items-center justify-center text-gray-900 text-lg font-bold disabled:opacity-40 shrink-0"
            >
              ↑
            </button>
          </div>
